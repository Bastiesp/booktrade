...
/* ════════════════════════════════════════════════
   NAVEGACIÓN
   ════════════════════════════════════════════════ */
/* Actualizar ítem activo y badge — el nav es estático, no se reconstruye */
function setActiveNav(view) {
  ['discover','books','matches','profile'].forEach(v => {
...
/* Alias para compatibilidad con llamadas existentes a renderNav() */
function renderNav() { setActiveNav(state.view); }
...
async function renderDiscover() {
  ...
  const content = document.getElementById('content');
  if (content) { content.innerHTML = ''; content.appendChild(viewEl); }
  else app().appendChild(viewEl);
...
async function startApp() {
  /* Estructura fija — nav hardcodeado con onclick, nunca falla */
  app().innerHTML = `
    <div id="content" style="flex:1;overflow-y:auto;overflow-x:hidden;min-height:0"></div>
    <div id="bottom-nav" class="bottom-nav">
      <button class="nav-item active" id="nav-discover" onclick="navigate('discover')">
        <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
          <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
        </svg>
        <span class="nav-item-label">Descubrir</span>
      </button>
      <button class="nav-item" id="nav-books" onclick="navigate('books')">
        <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
          <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20M4 19.5A2.5 2.5 0 0 0 6.5 22H20V2H6.5A2.5 2.5 0 0 0 4 4.5v15z"/>
        </svg>
        <span class="nav-item-label">Mis Libros</span>
      </button>
      <button class="nav-item" id="nav-matches" onclick="navigate('matches')">
        <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
          <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
        </svg>
        <span id="nav-dot" class="nav-dot" style="display:none"></span>
