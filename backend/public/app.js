...
function renderNav() {
  ...
  const hasDot = state.matchCount > 0 || totalUnread() > 0;
  ...
  nav.innerHTML = Object.entries(views).map(([v, icon]) => `
    <button class="nav-item ${v===state.view?'active':''}" data-view="${v}">
      ${icon}</svg>
      ${v==='matches' && hasDot ? `<span class="nav-dot"></span>` : ''}
...
async function renderMyBooks() {
  ...
  const fabContainer = document.getElementById('content') || app();
...
async function startApp() {
  /* Estructura fija: #content arriba, nav abajo — nunca se mueven */
  app().innerHTML = `
    <div id="content" style="flex:1;overflow:hidden;display:flex;flex-direction:column;min-height:0"></div>
