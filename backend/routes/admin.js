...
router.put('/users/:id/role',auth,requireAdmin,async(req,res)=>{
  try{
    ...
    const u=await User.findById(targetId).select('-password -resetPasswordToken -resetPasswordExpires');
    ...
    if(u.accountStatus==='deleted'){
      return res.status(400).json({error:'No puedes cambiar rol a un usuario eliminado'});
    }

    u.role=newRole;
    await u.save();

    await log(req,'change_role','user',targetId,{role:newRole,username:u.username,email:u.email});
