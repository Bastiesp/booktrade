...
router.post('/contact-admin',auth,async(req,res)=>{
  try{
    const {subject,message,reportedUserId,bookId,type}=req.body;
    const cleanMessage=String(message||'').trim();
    if(!cleanMessage)return res.status(400).json({error:'Debes escribir un mensaje para soporte'});
    const finalReason=`${subject?('Asunto: '+String(subject).slice(0,120)+'\n'):''}${cleanMessage.slice(0,1200)}`;
    const report=await Report.create({
      reporter:req.userId,
      reportedUser:reportedUserId||null,
      book:bookId||null,
      type:type||'contact_admin',
      reason:finalReason
    });
    res.status(201).json({ok:true,report});
  }catch(e){
    console.error('POST /api/support/contact-admin error:',e);
